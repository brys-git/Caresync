...
<div class="pr">
    ...
    <div id="pr-panel-regular">
        <form method="post" action="<?= site_url('branch-admin/cash-payment-record/save') ?>" id="pr-form-regular">
            <?= csrf_field() ?>
    ...
    <div id="pr-panel-initial" style="display:none;">
        <form method="post" action="<?= site_url('branch-admin/cash-payment-record/save') ?>" id="pr-form-initial">
            <?= csrf_field() ?>
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        <?php foreach ($approval_queue as $record):
                            $isVerified = (int) ($record['verified'] ?? 0) === 1;
                            $statusClass = $isVerified ? 'approved' : 'pending';
                            $statusLabel = $isVerified ? 'APPROVED' : 'PENDING';
                            $clientLabel = esc((string) ($record['client_name'] ?? 'Unknown'));
                        ?>
                        <tr>
                            <td>
                                <div class="pr-name-cell">
                                    <?= $clientLabel ?>
                                    <span class="pr-search-tag pr-search-tag--<?= $isVerified ? 'verified' : 'pending' ?>">
...
(function () {
    ...
    document.querySelectorAll('.pr-search-input').forEach(function (input) {
