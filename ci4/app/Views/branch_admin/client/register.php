...
<div class="rw">
    ...
    <form method="post" action="<?= base_url('branch-admin/client/register-submit') ?>" enctype="multipart/form-data" id="rw-form">
        <?= csrf_field() ?>
                ...
                <div>
                    ...
                    <div class="rw-card">
                        ...
                        <div class="rw-form-row rw-form-row--3">
                            ...
                            <div class="rw-group">
                                <label class="rw-label" for="coordinator">Coordinator <span>*</span></label>
                                <select class="rw-select" id="coordinator" name="coordinator" data-rw-req>
                                    <option value="">Select Coordinator</option>
                                    <?php
                                    $currentCoord = old('coordinator', (string) ($plan_holder['coordinator'] ?? ''));
                                    foreach ($coordinators ?? [] as $coord):
                                        $coordName = trim((string) ($coord['first_name'] ?? '') . ' ' . (string) ($coord['last_name'] ?? ''));
                                    ?>
                                        <option value="<?= esc($coordName) ?>" <?= $currentCoord === $coordName ? 'selected' : '' ?>><?= esc($coordName) ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            <div class="rw-group">
        ...
        <div class="rw-nav">
            ...
            <div class="rw-nav__right">
                <button type="button" class="rw-save-link" onclick="document.getElementById('rw-form').submit();">Save Draft</button>
