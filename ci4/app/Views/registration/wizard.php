...
$showAccountMode = (bool) ($show_account_mode ?? false);
$existingUsers = $existing_users ?? [];
$isClient = (bool) ($is_client ?? false);
$approvalsUrl = (string) ($approvals_url ?? '');
?>
...
<div class="rw">
    ...
    <?php if ($approvalsUrl !== ''): ?>
        <div style="display:flex;justify-content:flex-end;margin-bottom:12px;">
            <a href="<?= esc($approvalsUrl) ?>" class="rw-btn rw-btn--outline" style="font-size:0.82rem;padding:6px 12px;">
                <i class="mdi mdi-clipboard-check-outline"></i> Approvals Queue
            </a>
        </div>
    <?php endif; ?>
    ...
    <form method="post" action="<?= esc($formAction) ?>" enctype="multipart/form-data" id="rw-form">
        <?= csrf_field() ?>
        ...
        <div class="rw-panel" data-rw-panel="1">
            ...
            <?php if ($showAccountMode): ?>
            <!-- ====== Account Mode (staff-assisted only) ====== -->
            <div class="rw-card" id="account_mode_card" style="margin-bottom:16px;">
                <h3 class="rw-card__title">Client Account</h3>
                <div class="rw-form-row rw-form-row--2">
                    <div class="rw-group">
                        <label class="rw-label">Account Status <span>*</span></label>
                        <div style="display:flex;flex-direction:column;gap:8px;padding-top:6px;">
                            <label class="rw-radio-line"><input type="radio" name="registration_mode" value="existing" <?= old('registration_mode', 'existing') === 'existing' ? 'checked' : '' ?> onchange="rwToggleAccountMode()"> Link an existing client account</label>
                            <label class="rw-radio-line"><input type="radio" name="registration_mode" value="new" <?= old('registration_mode') === 'new' ? 'checked' : '' ?> onchange="rwToggleAccountMode()"> Create a new client account</label>
                        </div>
                    </div>
                    <div class="rw-group" id="rw_existing_user_group">
                        <label class="rw-label" for="existing_user_id">Existing Client Account <span>*</span></label>
                        <select class="rw-select" id="existing_user_id" name="user_id" data-rw-req onchange="rwAutofillExisting()">
                            <option value="">Select an existing client account</option>
                            <?php foreach ($existingUsers as $eu): ?>
                                <?php $euName = trim((string) ($eu['first_name'] ?? '') . ' ' . (string) ($eu['middle_name'] ?? '') . ' ' . (string) ($eu['last_name'] ?? '')); ?>
                                <option value="<?= (int) ($eu['user_id'] ?? 0) ?>"
                                    data-first="<?= esc((string) ($eu['first_name'] ?? '')) ?>"
                                    data-middle="<?= esc((string) ($eu['middle_name'] ?? '')) ?>"
                                    data-last="<?= esc((string) ($eu['last_name'] ?? '')) ?>"
                                    data-email="<?= esc((string) ($eu['email'] ?? '')) ?>"
                                    data-contact="<?= esc((string) ($eu['contact_number'] ?? '')) ?>"><?= esc($euName !== '' ? $euName . ' (' . ($eu['email'] ?? '') . ')' : ($eu['email'] ?? '')) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="rw-group" id="rw_new_username_group" style="display:none;">
                        <label class="rw-label" for="username">Username <span>*</span></label>
                        <input class="rw-input" id="username" name="username" value="<?= esc(old('username')) ?>" autocomplete="off">
                    </div>
                    <div class="rw-group" id="rw_new_password_group" style="display:none;">
                        <label class="rw-label" for="password">Password <span>*</span></label>
                        <input class="rw-input" id="password" name="password" type="password" value="" autocomplete="new-password">
                    </div>
                    <div class="rw-group" id="rw_new_password2_group" style="display:none;">
                        <label class="rw-label" for="password_confirm">Confirm Password <span>*</span></label>
                        <input class="rw-input" id="password_confirm" name="password_confirm" type="password" value="" autocomplete="new-password">
                    </div>
                </div>
                <p class="rw-spouse-note" style="margin-top:10px;">
                    <i class="mdi mdi-information-outline"></i> Existing accounts must be client accounts that do not yet have a plan. Creating a new account sets the login credentials the client will use.
                </p>
            </div>
            <?php endif; ?>
        ...
        <div class="rw-panel" data-rw-panel="4" style="display:none;">
            ...
            <div class="rw-section-sub"><?= $isClient ? 'Submit your initial payment to activate your membership.' : 'The initial payment will be recorded by staff to activate the membership.' ?></div>
            ...
            <div class="rw-card">
                ...
                <div class="rw-info-box" style="margin-top:12px;">
                    ...
                    <?php else: ?>
                    <p style="margin:0;">After this registration is saved, record the client's <strong>initial payment</strong> (GCash reference or cash receipt) in the payment tracking screen. Once the payment is verified and approved, the membership is activated automatically.</p>
                    <?php endif; ?>
                </div>
            </div>
        </div>
...
(function () {
    ...
    window.rwNav = function (dir) {
        if (dir === 1) {
            ...
            /* Account-mode: validate password confirmation when creating a new account */
            if (currentStep === 1) {
                var acctMode = document.querySelector('input[name="registration_mode"]:checked');
                if (acctMode && acctMode.value === 'new') {
                    var pw1 = document.getElementById('password');
                    var pw2 = document.getElementById('password_confirm');
                    if (pw1 && pw2 && String(pw1.value || '').trim() !== '' && String(pw1.value || '') !== String(pw2.value || '')) {
                        alert('Password and Confirm Password must match.');
                        pw2.focus();
                        pw2.style.borderColor = 'var(--rw-red)';
                        setTimeout(function () { pw2.style.borderColor = ''; }, 2000);
                        return;
                    }
                }
            }
    ...
    /* ------------------------------------------------------------------ */
    /* Account mode (existing/new) — staff-assisted flows                  */
    /* ------------------------------------------------------------------ */
    window.rwToggleAccountMode = function () {
        var card = document.getElementById('account_mode_card');
        if (!card) return;
        var checked = document.querySelector('input[name="registration_mode"]:checked');
        var mode = (checked && checked.value) || 'existing';

        var existingGroup = document.getElementById('rw_existing_user_group');
        var newUsername = document.getElementById('rw_new_username_group');
        var newPass = document.getElementById('rw_new_password_group');
        var newPass2 = document.getElementById('rw_new_password2_group');

        if (existingGroup) existingGroup.style.display = mode === 'existing' ? '' : 'none';
        if (newUsername) newUsername.style.display = mode === 'new' ? '' : 'none';
        if (newPass) newPass.style.display = mode === 'new' ? '' : 'none';
        if (newPass2) newPass2.style.display = mode === 'new' ? '' : 'none';

        /* Toggle data-rw-req so rwNav only validates active fields */
        var existingSel = document.getElementById('existing_user_id');
        if (existingSel) {
            if (mode === 'existing') existingSel.setAttribute('data-rw-req', '1');
            else existingSel.removeAttribute('data-rw-req');
        }
        ['username', 'password', 'password_confirm'].forEach(function (id) {
            var el = document.getElementById(id);
            if (el) {
                if (mode === 'new') el.setAttribute('data-rw-req', '1');
                else el.removeAttribute('data-rw-req');
            }
        });
    };

    window.rwAutofillExisting = function () {
        var sel = document.getElementById('existing_user_id');
        if (!sel) return;
        var opt = sel.options[sel.selectedIndex];
        if (!opt || !opt.value) return;
        var set = function (id, val) {
            var el = document.getElementById(id);
            if (el) { el.value = val || ''; el.setAttribute('readonly', 'readonly'); }
        };
        set('first_name', opt.dataset.first);
        set('middle_name', opt.dataset.middle);
        set('last_name', opt.dataset.last);
        set('email', opt.dataset.email);
        set('contact_number', opt.dataset.contact);
    };

    /* Initialize account-mode visibility on load */
    if (document.getElementById('account_mode_card')) {
        rwToggleAccountMode();
    }
})();
</script>
