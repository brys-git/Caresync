...
<div class="container-fluid py-3">
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <div class="no-print d-flex gap-2">
            <form method="post" action="<?= site_url('/admin/reports/remittance/generate') ?>" class="d-inline">
                <?= csrf_field() ?>
                ...
                <input type="hidden" name="branch_id" value="<?= esc((string) ($filters['branch_id'] ?? 0)) ?>">
    ...
    <div class="card mb-3 no-print">
        <div class="card-body">
            <form method="post" action="<?= site_url('/admin/reports/remittance/generate') ?>">
                <?= csrf_field() ?>
                <div class="row g-3 align-items-end">
                    <div class="col-md-2">
                        <label class="form-label">Branch</label>
                        <select class="form-select" name="branch_id">
                            <option value="0" <?= (int) ($filters['branch_id'] ?? 0) === 0 ? 'selected' : '' ?>>All Branches</option>
                    ...
                    <div class="col-md-2">
                        ...
                        <select class="form-select" name="received_by">
                            ...
                            <?php foreach (($staff_options ?? []) as $staff): ?>
                                <option value="<?= (int) $staff['user_id'] ?>" <?= (int) ($filters['received_by'] ?? 0) === (int) $staff['user_id'] ? 'selected' : '' ?>>
                ...
                <table class="table table-striped mb-0">
                    ...
                    <tbody>
                        <?php if (empty($report_rows ?? [])): ?>
                            <tr><td colspan="8" class="text-center py-3">No remittance transactions for selected filters.</td></tr>
                        <?php else: ?>
